...
export const itemMeta: ItemMeta = {
  icon: ItemIcon,
  getDescription: (item) =>
    `**After Skip**: If the Understand Chance of the next lecture is less than the Understand Chance of this one, carry it over and add **${Math.min(item.level / 10, 100).toFixed(2)}%**`,
...
export const itemBehavior: ItemBehavior = {
  afterSkipLecture: (params) =>
  {
    ...
    if (params.state.nextLecture.understandChance < params.lecture.understandChance)
    {
      ...
      params.state.nextLecture.understandChance = params.lecture.understandChance + Math.min(params.item.level / 10 / 100, 1);
